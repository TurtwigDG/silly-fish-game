...
func _ready() -> void:
	...
	upgrades = {"Sonar": false, "Shark": false}
...
func _process(delta: float) -> void:
	...
	if Input.is_action_just_pressed("dash") and dashCooldownTimer >= dashCooldownWindow:
		...
		if !upgrades["Shark"]:
			...
			sprite.get_node("DashSprite").visible = true
			currentSwimSpeed = sharkSwimSpeed
		
	if Input.is_action_just_released("dash"):
		#sprite.get_node("DashSprite").visible = false
		currentSwimSpeed = slowSwimSpeed
